chrome.runtime.onInstalled.addListener(function () {
    chrome.storage.sync.get("memos", (items) => {
        if (Object.keys(items).length) {
            chrome.storage.sync.remove("memos");
            chrome.storage.sync.set({ "slist": Object.keys(items.memos), ...items.memos });
        }
    })
    chrome.storage.sync.get("simple_memo", function (items) {
        ...
        else if (typeof items.simple_memo == "string") {
...
function listen_mes() {
    ...
    return function (items) {
        ...
        timer = setTimeout(function () {
            ...
            chrome.storage.sync.set({ "slist": Object.keys(items[0]["sync"]), ...items[0]["sync"] });
        }, 2000)
    }
}